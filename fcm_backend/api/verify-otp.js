...
async function logSecurityEvent(params) {
  ...
  try {
    ...
    console.error('[Security Logger Error in fcm_backend verifyOtp]:', err.message);
  }
}
  ...
  try {
    ...
    if (rpcErr) {
      await logSecurityEvent({
        ...
        responseStatus: 500,
        authResult: 'DENIED',
        details: { phone: maskPhone(cleanPhone), errorMessage: rpcErr.message }
      });
      return res.status(500).json({ success: false, error: 'حدث خطأ أثناء التحقق من رمز التأكيد في قاعدة البيانات.' });
    }

    if (!verifyRes || verifyRes.valid !== true) {
      return res.status(400).json({ success: false, error: verifyRes?.error || 'رمز التحقق غير صحيح أو انتهت صلاحيته.' });
    }
